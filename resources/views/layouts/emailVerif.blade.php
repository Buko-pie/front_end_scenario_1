<!doctype html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    
    <!-- CSRF Token -->
    {{-- <meta name="csrf-token" content="{{ csrf_token() }}"> --}}

    <title>{{ config('app.name', 'Laravel') }}</title>

    <!-- Scripts -->
    <script src="{{ asset('js/app.js') }}" defer></script>
    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
    <script src="https://unpkg.com/vue/dist/vue.js"></script>
    <script src="https://unpkg.com/sweetalert/dist/sweetalert.min.js"></script>
    <script src="https://unpkg.com/axios/dist/axios.min.js"></script>
    <script src="//unpkg.com/bootstrap-vue@latest/dist/bootstrap-vue.min.js"></script>
    <!-- Fonts -->
    <link rel="dns-prefetch" href="//fonts.gstatic.com">
    <link href="https://fonts.googleapis.com/css?family=Nunito" rel="stylesheet">

    <!-- Styles -->
    <link href="{{ asset('css/app.css') }}" rel="stylesheet">

    <style>
        .bg-deep_pruple{
            background: #673AB7;  /* fallback for old browsers */
            background: -webkit-linear-gradient(to right, #512DA8, #673AB7);  /* Chrome 10-25, Safari 5.1-6 */
            background: linear-gradient(to right, #512DA8, #673AB7); /* W3C, IE 10+/ Edge, Firefox 16+, Chrome 26+, Opera 12+, Safari 7+ */
        }
        .bg-amin{
            background: #8E2DE2;  /* fallback for old browsers */
            background: -webkit-linear-gradient(to right, #4A00E0, #8E2DE2);  /* Chrome 10-25, Safari 5.1-6 */
            background: linear-gradient(to right, #4A00E0, #8E2DE2); /* W3C, IE 10+/ Edge, Firefox 16+, Chrome 26+, Opera 12+, Safari 7+ */
        }
        .btn-violet {
            color: #fff;
            background-color: rgb(102, 51, 153);
            border-color: rgb(102, 51, 153);
        }
        .btn-violet:hover {
            color: #fff;
            background-color: rgb(87, 18, 156);
            border-color: rgb(87, 18, 156);
        }
        .btn-violet:focus, .btn-violet.focus {
            color: #fff;
            background-color: rgb(87, 18, 156);
            border-color: rgb(87, 18, 156);
            box-shadow: 0 0 0 0.2rem rgba(103, 0, 187, 0.5);
        }
        .btn-violet.disabled, .btn-violet:disabled {
            color: #fff;
            background-color: #5d478a;
            border-color: #5d478a;
        }
        .btn-violet:not(:disabled):not(.disabled):active, .btn-violet:not(:disabled):not(.disabled).active, .show > .btn-violet.dropdown-toggle {
            color: #fff;
            background-color: rgb(87, 18, 156);
            border-color: rgb(87, 18, 156);
        }
        .btn-violet:not(:disabled):not(.disabled):active:focus, .btn-violet:not(:disabled):not(.disabled).active:focus, .show > .btn-violet.dropdown-toggle:focus {
            box-shadow: 0 0 0 0.2rem rgba(103, 0, 187, 0.5);
        }

        .btn-violet-outline {
            color: rgb(102, 51, 153);
            background-color: rgb(255, 255, 255);
            border-color: rgb(102, 51, 153);
        }
        .btn-violet-outline:hover {
            color: rgb(102, 51, 153);
            background-color: #dbdbdb;
            border-color: rgb(102, 51, 153);
        }
        .btn-violet-outline:focus, .btn-violet.focus {
            color: rgb(102, 51, 153);
            background-color: #dbdbdb;
            border-color: rgb(102, 51, 153);
            box-shadow: 0 0 0 0.2rem rgba(103, 0, 187, 0.5);
        }
        .btn-violet-outline.disabled, .btn-violet:disabled {
            color: #797979;
            background-color: #ffffff;
            border-color: #797979;
        }
        .btn-violet-outline:not(:disabled):not(.disabled):active, .btn-violet:not(:disabled):not(.disabled).active, .show > .btn-violet.dropdown-toggle {
            color: rgb(102, 51, 153);
            background-color: #fff;
            border-color: rgb(102, 51, 153);
        }
        .btn-violet-outline:not(:disabled):not(.disabled):active:focus, .btn-violet:not(:disabled):not(.disabled).active:focus, .show > .btn-violet.dropdown-toggle:focus {
            box-shadow: 0 0 0 0.2rem rgba(103, 0, 187, 0.5);
        }
        .text-purple{
            color: purple;
        }
        .pill{
            border-radius: 500px;
        }
        .spaced {
            letter-spacing: 0.5rem;
        }
    </style>
    {{-- <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous"> --}}

</head>
<body>
    <div id="app">
        <main class="py-4">
            @yield('content')
        </main>
    </div>
</body>
</html>
